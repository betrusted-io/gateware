...
impl JtagPhy for JtagGpioPhy {
    ...
    fn sync(&mut self, tdi: bool, tms: bool) -> bool {
        ...
        while !p.JTAG.tdo.read().ready().bit() { }  // make sure we are in a ready/tdo valid state
        let ret = p.JTAG.tdo.read().tdo().bit(); // grab the tdo value /prior/ to clocking the new bit
        p.JTAG.next.write(|w| w.tdi().bit(tdi).tms().bit(tms) ); // update tdi/tms, which automatically clocks tck
