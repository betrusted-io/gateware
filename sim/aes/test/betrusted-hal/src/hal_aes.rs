...
impl BtAes {
    ...
    pub fn key_put(&mut self, key: &mut &[u8] ) -> bool {
        match key.len() {
            16 => unsafe{ self.p.AES.ctrl.write( |w| {w.key_len().bits(AesCtrl::KEY_LEN_128.bits() as u8)}) },
            24 => unsafe{ self.p.AES.ctrl.write( |w| {w.key_len().bits(AesCtrl::KEY_LEN_192.bits() as u8)}) },
            32 => unsafe{ self.p.AES.ctrl.write( |w| {w.key_len().bits(AesCtrl::KEY_LEN_256.bits() as u8)}) },
            _ => return false,
        }
    ...
    pub fn aes_setup(&mut self) {
        unsafe{ self.p.AES.trigger.write(|w|{ w.bits(0)}); }
    }
