...
impl BtAes {
    ...
    pub fn report(&mut self, report: u32) {
        unsafe {self.p.SIMSTATUS.report.write(|w|{w.bits(report)});}
    }
    ...
    pub fn aes_data_put(&mut self, data: &mut &[u8]) -> bool {
        ...
        for( reg, chunk) in data.chunks(4).enumerate() {
            ...
            match reg {
                ...
                _ => { unsafe{ self.p.SIMSTATUS.report.write(|w| w.bits(reg as u32));} assert!(false)},
            }
        }
        true
    }
